...
const ProductCard = ({ product, isInCart }) => {
  let { setCartItems, incrementQuantity, decrementQuantity } = useContext(MyEComm);
  ...
  return (
    <div className="group bg-white rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-200 hover:-translate-y-2">
      ...
      <div className="p-5 flex flex-col gap-3">
        ...
        {/* Button */}
        {isInCart ? (
          <button className="mt-2 bg-black flex justify-center items-center gap-5 text-white py-3 rounded-xl font-semibold hover:bg-gray-800 transition duration-300 cursor-pointer">
            <span  onClick={()=> decrementQuantity(product.id)} className="text-xl">-</span>
            <span className="text-xl">{isInCart.quantity}</span>
            <span onClick={()=> incrementQuantity(product.id)} className="text-xl">+</span>
          </button>
        ) : (
          <button
            onClick={addToCart}
